...
{
	$sql = "select product_name from products where product_id = ".$id." AND product_status =1 order by product_created_date desc";
	$results = get_row($sql);
	return $results;
}
...
{
	$sql = "SELECT COUNT(*) FROM products where product_status =1 ";
	$results = get_row($sql);
...
{
	$sql = "select * from products where product_status =1  and product_admin_status = 0 order by product_created_date desc";
	$results = get_results($sql);
	return $results;
}
...
function get_product_bycat($cat)
{
	$sql = "select * from products where product_status =1 and product_category =".$cat;
	$results = get_results($sql);
	return $results;
}
?>